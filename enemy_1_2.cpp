...
Enemy::Enemy() : Entity("Enemy"), baseHp_(10), baseAtk_(1), baseDef_(0), curHp_(10), atk_(1), def_(0), level_(1), tier_(EnemyTier::Normal) {}
Enemy::Enemy(std::string name, int baseHp, int baseAtk, int baseDef, int level, EnemyTier tier)
    : Entity(std::move(name)), baseHp_(baseHp), baseAtk_(baseAtk), baseDef_(baseDef), curHp_(baseHp), atk_(baseAtk), def_(baseDef), level_(level), tier_(tier) {}
...
void Enemy::applyDayScaling(int days) {
    ...
    atk_ = static_cast<int>(std::ceil(baseAtk_ * atkMul));
    def_ = static_cast<int>(std::ceil(baseDef_ * defMul));
    curHp_ = static_cast<int>(std::ceil(baseHp_ * hpMul));
}

void Enemy::applyExtraFromBoss(const Enemy& lastBoss) {
    atk_ += static_cast<int>(std::ceil(0.5 * lastBoss.atk()));
    def_ += static_cast<int>(std::ceil(0.5 * lastBoss.def()));
    curHp_ += static_cast<int>(std::ceil(0.5 * lastBoss.hp()));
}
